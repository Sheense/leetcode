...
public class 拼接最大数 {
    public static void main(String[] args) {
        ...
        ArrayUtils.print(s.maxNumber(new int[]{5,0,2,1,0,1,0,3,9,1,2,8,0,9,8,1,4,7,3},
                new int[]{7,6,7,1,0,1,0,5,6,0,5,0}, 31));
        ArrayUtils.print(s.maxNumber(new int[]{2,5,6,4,4,0},
    ...
    public int[] maxNumber(int[] nums1, int[] nums2, int k) {
        ...
        for(int i = 0; i <= k; i++) {
            if(nums1.length >= i && nums2.length >= (k - i)) {
                ...
                int[] merge = merge(a, b, k);
                if(isBig(merge, res, 0, 0)) {
                    res = merge;
                }
            }
        }
    ...
    //遇到相等的元素，就看下后面不相等的元素那个最大，就选那个，贪心思想
    public boolean isBig(int [] a, int[] b, int i, int j) {
    ...
    public int[] findMax(int[] nums, int size) {
        ...
        for(int i = 0; i < nums.length; i++) {
            while (!stack.isEmpty() && stack.peek() < nums[i] && stack.size() + nums.length - i - 1 >= size) {
                stack.pop();
            }
            stack.push(nums[i]);
            if(stack.size() + nums.length - i - 1 == size) {
