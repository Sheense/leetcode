...
public class 连接词 {
    ...
    public List<String> findAllConcatenatedWordsInADict(String[] words) {
        ...
        for(int i = 0; i < words.length; i++) {
            ...
            if(words[i].equals("ta")){
                System.out.println(1);
            }
            if(dfs(tree, words[i], 0)) {
                res.add(words[i]);
            }else {
                insert(tree, words[i], 0);
            }
        }
    ...
    public void insert(Node[] tree, String str, int index) {
        int key = str.charAt(index) - 'a';
        if(tree[key] == null) {
            tree[key] = new Node();
        }
        Node[] next = tree[key].next;
        index ++;
        if(index == str.length()) {
            tree[key].end = true;
        }else {
