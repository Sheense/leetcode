...
*/
    ...
    public int maxProfit(int k, int[] prices) {
        ...
        for(int i=0;i<dp.length;i++) {
            for(int j=0;j<dp[i].length;j++) {
                if(j==0) {
                    ...
                    if(i==0) {
                        dp[i][j][1] = Math.max(dp[i][j-1][1], -prices[j]);
                    }else {
        ...
        return dp[k-1][prices.length-1][0];
    }

    public int greet(int[] prices){
